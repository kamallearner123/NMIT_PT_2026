...
let currentWeightType = 'weighted';
...
function generateRandomGraph() {
    ...
    const weightSelect = document.getElementById('weightType');
    currentGraphType = typeSelect ? typeSelect.value : 'undirected';
    currentWeightType = weightSelect ? weightSelect.value : 'weighted';
    ...
    for (let i = 0; i < numNodes; i++) {
        for (let j = 0; j < numNodes; j++) {
            if (i === j) continue;
            if (currentGraphType === 'undirected' && i > j) continue;

            if (Math.random() > 0.65) {
                const weight = currentWeightType === 'weighted' ? Math.floor(Math.random() * 12) + 1 : 1;
                adjacencyList[i].push({to: j, weight: weight});
                if (currentGraphType === 'undirected') {
    ...
    for (let i = 0; i < numNodes; i++) {
        ...
        if (!adjacencyList[i].some(e => e.to === j) && !adjacencyList[j].some(e => e.to === i)) {
            const weight = currentWeightType === 'weighted' ? Math.floor(Math.random() * 10) + 1 : 1;
            adjacencyList[i].push({to: j, weight: weight});
            if (currentGraphType === 'undirected') {
                adjacencyList[j].push({to: i, weight: weight});
            }
...
function renderGraph(activeNodeId = null, visitedNodes = [], queue = [], distances = {}, activeEdges = []) {
    ...
    if (!svg.querySelector('defs')) {
        ...
        <defs>
            ...
            <marker id="arrowhead-active" markerWidth="10" markerHeight="7" refX="34" refY="3.5" orient="auto">
    ...
    for (let u = 0; u < nodes.length; u++) {
        adjacencyList[u].forEach(edge => {
            if (currentGraphType === 'directed' || u < edge.to) { // Draw undirected edges once
                const v = edge.to;
                const nodeU = nodes[u];
                const nodeV = nodes[v];
                
                const isActive = activeEdges.some(ae => 
                    (ae.from === u && ae.to === v) || (currentGraphType === 'undirected' && ae.from === v && ae.to === u)
                );
...
function startVisualization() {
    ...
    if ((algo === 'prim' || algo === 'kruskal') && currentGraphType === 'directed') {
        alert("Error: Minimum Spanning Tree algorithms (Prim's, Kruskal's) are designed for undirected graphs. Please choose an undirected graph or a different algorithm.");
        return;
    }
